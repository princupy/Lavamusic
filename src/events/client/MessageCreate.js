const { Event, Lavamusic, Context } = require('../../structures/index.js');
const { Message, PermissionFlagsBits, Collection, ChannelType } = require('discord.js');
...
module.exports = class MessageCreate extends Event {
  ...
  async run(message) {
    ...
    if (!message.inGuild() || !message.channel.permissionsFor(message.guild.members.me).has(PermissionFlagsBits.VIEW_CHANNEL))
      return;

    if (!message.guild.members.me.permissions.has(PermissionFlagsBits.SEND_MESSAGES))
      return await message.author.send({
        content: `I don't have **\`SendMessage\`** permission in \`${message.guild.name}\`\nchannel: <#${message.channelId}>`,
      }).catch(() => { });

    if (!message.guild.members.me.permissions.has(PermissionFlagsBits.EMBED_LINKS))
    ...
    if (command.player) {
      if (command.player.voice) {
        ...
        if (message.member.voice.channel.type === ChannelType.GUILD_STAGE_VOICE &&
          !message.guild.members.me.permissions.has(PermissionFlagsBits.REQUEST_TO_SPEAK))
          return await message.reply({
